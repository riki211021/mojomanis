...
<div class="bg-gray-50 min-h-screen py-10 px-6">
    <div class="max-w-6xl mx-auto space-y-10">
        <div class="bg-gradient-to-r from-blue-700 to-blue-500 text-white p-8 rounded-2xl shadow-lg">
            <h1 class="text-3xl md:text-4xl font-extrabold mb-2">Data Warga Negara</h1>
            <p class="text-blue-100">Statistik berdasarkan kewarganegaraan</p>
        </div>

        <div class="flex justify-end gap-2">
            <button id="togglePie" class="bg-blue-600 text-white px-4 py-2 rounded-lg">Pie Chart</button>
            <button id="toggleBar" class="bg-green-600 text-white px-4 py-2 rounded-lg">Bar Chart</button>
        </div>

        <div class="bg-white p-6 rounded-2xl shadow-lg">
            <div id="wargaNegaraChart"></div>
        </div>
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function renderChart(type) {
        Highcharts.chart('wargaNegaraChart', {
            chart: { type: type === 'pie' ? 'pie' : 'column' },
            title: { text: 'Distribusi Warga Negara' },
            plotOptions: {
