...
<div class="bg-gray-50 min-h-screen py-10 px-6">
    <div class="max-w-6xl mx-auto space-y-10">
        ...
        <div class="bg-gradient-to-r from-blue-700 to-blue-500 text-white p-8 rounded-2xl shadow-lg">
            ...
            <p class="text-blue-100">Statistik pendidikan berdasarkan data KK di Desa Mojomanis</p>
        </div>
    ...
    <table class="w-full border text-sm">
        <thead>
            <tr class="bg-gray-200 text-center font-bold">
                <th class="border px-4 py-2">kode</th> {{-- ganti dari "Kode" ke nomor urut --}}
        ...
        <tbody>
            ...
            @empty
                <tr>
                    <td colspan="4" class="text-center py-4 text-gray-500">Belum ada data pendidikan</td>
                </tr>
            @endforelse
        </tbody>
        @if($total > 0)
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    // Kalau datanya kosong, jangan render chart
    if (!chartData || chartData.length === 0) {
        document.getElementById('pendidikanChart').innerHTML =
            "<p class='text-center text-gray-500'>Belum ada data untuk ditampilkan</p>";
        return;
    }

    function renderChart(type) {
        Highcharts.chart('pendidikanChart', {
            chart: { type: type === 'pie' ? 'pie' : 'column' },
            title: { text: 'Distribusi Pendidikan' },
            tooltip: {
