...
<div class="bg-gray-50 min-h-screen py-10 px-6">
    <div class="max-w-6xl mx-auto space-y-10">
        ...
        <div class="bg-gradient-to-r from-blue-700 to-blue-500 text-white p-8 rounded-2xl shadow-lg">
            <h1 class="text-3xl md:text-4xl font-extrabold mb-2">Data Pekerjaan</h1>
            <p class="text-green-100">Statistik pekerjaan masyarakat Desa Mojomanis</p>
        </div>

        {{-- Toggle Chart --}}
        <div class="flex justify-end gap-2">
            <button id="togglePie" class="bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700">Pie Chart</button>
            <button id="toggleBar" class="bg-green-600 text-white px-4 py-2 rounded-lg hover:bg-green-700">Bar Chart</button>
        </div>
...
<div class="bg-white p-6 rounded-2xl shadow-lg overflow-x-auto">
    <table class="w-full border text-sm">
        ...
        <tbody>
            @foreach($data as $row)
                <tr>
                    {{-- ✅ Kode dari database, bukan nomor urut --}}
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function renderChart(type) {
        Highcharts.chart('pekerjaanChart', {
            chart: { type: type === 'pie' ? 'pie' : 'column', height: 500 },
            title: { text: 'Distribusi Pekerjaan' },
            tooltip: { pointFormat: '{series.name}: <b>{point.y} ({point.percentage:.2f}%)</b>' },
            plotOptions: {
                pie: {
                    innerSize: '50%',
            ...
            series: [{ name: 'Jumlah', colorByPoint: true, data: chartData }]
        });
    }
