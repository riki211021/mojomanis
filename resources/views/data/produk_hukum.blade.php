...
<div class="bg-gray-50 min-h-screen py-10 px-6">
    <div class="max-w-6xl mx-auto space-y-10">
        ...
        {{-- Header --}}
        <div class="bg-gradient-to-r from-blue-700 to-blue-500 text-white p-10 rounded-3xl shadow-lg text-center">
            <h1 class="text-3xl md:text-4xl font-extrabold tracking-tight">Produk Hukum Desa Mojomanis</h1>
            <p class="text-red-100 mt-2 text-sm md:text-base">
                Kumpulan peraturan, keputusan, dan kebijakan resmi Desa Mojomanis
            </p>
        </div>
        ...
        <div class="bg-white p-6 rounded-2xl shadow-md flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div class="flex items-center gap-3 w-full md:w-auto">
                ...
                <select id="filterTahun"
                        class="border border-gray-300 rounded-xl px-3 py-2 focus:outline-none focus:ring-2 focus:ring-red-400 w-full md:w-40">
            ...
            <div class="flex items-center gap-3 w-full md:w-auto">
                <label for="filterJenis" class="text-gray-600 font-medium">Jenis</label>
                <select id="filterJenis"
                        class="border border-gray-300 rounded-xl px-3 py-2 focus:outline-none focus:ring-2 focus:ring-red-400 w-full md:w-52">
                    <option value="">Semua</option>
                    @foreach($data->pluck('jenis')->unique()->sort() as $jenis)
            ...
            <div class="relative w-full md:w-1/2">
                <input type="text" id="searchInput" placeholder="Cari judul produk hukum..."
                       class="border border-gray-300 rounded-xl w-full px-4 py-2 pl-10 focus:outline-none focus:ring-2 focus:ring-red-400">
        ...
        <div class="bg-white p-6 rounded-3xl shadow-lg overflow-x-auto">
            <table id="produkHukumTable" class="w-full border-collapse text-sm text-gray-700">
                <thead>
                    <tr class="bg-red-100 text-gray-800 text-center font-bold uppercase">
                        <th class="border px-4 py-3 w-12">No</th>
                        <th class="border px-4 py-3 text-left">Judul Produk Hukum</th>
                        <th class="border px-4 py-3 w-32">Jenis</th>
                ...
                <tbody>
                    ...
                    <tr class="hover:bg-red-50 transition duration-200">
                        ...
                        <td class="border px-4 py-2 text-center capitalize">{{ $row->jenis ?? '-' }}</td>
   ...
   download
   class="inline-flex items-center gap-2 bg-blue-600 text-white px-3 py-1.5 rounded-lg shadow hover:bg-blue-700 transition">
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const rows = document.querySelectorAll('#produkHukumTable tbody tr');
    ...
    function filterTable() {
        ...
        rows.forEach(row => {
            ...
            const jenisText = row.children[2].innerText.toLowerCase();
            const judulText = row.children[1].innerText.toLowerCase();

            const matchTahun = !tahun || tahunText.includes(tahun);
            const matchJenis = !jenis || jenisText.includes(jenis);
            const matchSearch = judulText.includes(search);

            row.style.display = matchTahun && matchJenis && matchSearch ? '' : 'none';
        });
    }
